...
x_1 %>% describe %>% select(c("sd","mean","median","min","max")) %>% round(2) %>% write.csv("out_2.csv",quote=FALSE)
...
x_2 %>%  select(names) %>% purrr::map(~boxplot.stats(.x,coef = 1)$out) # sąlyginės išskirtys ("mild" outliers)
...
ggbetweenstats(data = x_2,
               ...
               outlier.tagging = TRUE, outlier.label = "Name",pairwise.comparisons = FALSE,point.args = list(position="jitter")) 
```
